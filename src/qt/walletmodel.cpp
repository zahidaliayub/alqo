...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered alqo address / amount:
...

    {
        ...
        if (recipients[0].useHyperSend && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("HyperSend doesn't support sending values that high yet. Transactions are currently limited to %1 XLQ.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }

        bool fCreated = wallet->CreateTransaction(vecSend, *newTx, *keyChange, nFeeRequired, strFailReason, coinControl, recipients[0].inputType, recipients[0].useHyperSend);
        transaction.setTransactionFee(nFeeRequired);

        if (recipients[0].useHyperSend && newTx->GetValueOut() > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("HyperSend doesn't support sending values that high yet. Transactions are currently limited to %1 XLQ.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
                CClientUIInterface::MSG_ERROR);
            return TransactionCreationFailed;
        }
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal alqo:URI (alqo:XyZ...?message=example)
            {
        ...
        if (!wallet->CommitTransaction(*newTx, *keyChange, (recipients[0].useHyperSend) ? "ix" : "tx"))
...
static void NotifyzXLQReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzXLQReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzXLQReset.connect(boost::bind(NotifyzXLQReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzXLQReset.disconnect(boost::bind(NotifyzXLQReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zXLQTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
