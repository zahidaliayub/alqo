...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::ALQO)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeALQOAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeALQOAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeALQOAmount = strAnonymizeALQOAmount.remove(strAnonymizeALQOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeALQOAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeALQOAmount * COIN) nMaxToAnonymize = nAnonymizeALQOAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeALQOAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeALQOAmount));
        strAnonymizeALQOAmount = strAnonymizeALQOAmount.remove(strAnonymizeALQOAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeALQOAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
