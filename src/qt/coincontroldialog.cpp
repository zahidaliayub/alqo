...
void CoinControlDialog::updateLabels(WalletModel* model, QDialog* dialog)
{
    ...
    if (nQuantity > 0) {
        ...
        // IX Fee
        if (coinControl->useHyperSend) nPayFee = max(nPayFee, CENT);
        // Allow free?
    ...
    QString toolTip4 = tr("Can vary +/- %1 uXLQ per input.").arg(dFeeVary);
    ...
    BOOST_FOREACH (PAIRTYPE(QString, vector<COutput>) coins, mapCoins) {
        ...
        for(const COutput& out: coins.second) {
            ...
            if (ExtractDestination(out.tx->vout[out.i].scriptPubKey, outputAddress)) {
                ...
                // if listMode or change => show ALQO address. In tree mode, address is not shown again for direct wallet address outputs
