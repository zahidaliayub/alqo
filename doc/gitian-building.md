...
*Setup instructions for a gitian build of ALQO using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ALQO
...
Clone the git repositories for alqo and gitian and then checkout the alqo version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/alqo-crypto/alqo.git
cd alqo
...
Follow the instructions in [doc/release-process.md](release-process.md) in the alqo repository
...
gitian builds' in [doc/release-process.md](release-process.md) in the alqo repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/alqo/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/alqo-crypto/alqo
...
For example:
```bash
URL=https://github.com/crowning-/alqo.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit alqo=${COMMIT} --url alqo=${URL} ../alqo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit alqo=${COMMIT} --url alqo=${URL} ../alqo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit alqo=${COMMIT} --url alqo=${URL} ../alqo/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/alqo-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/alqo-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/alqo-build.assert
```
...
[alqo/gitian.sigs](https://github.com/alqo-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
